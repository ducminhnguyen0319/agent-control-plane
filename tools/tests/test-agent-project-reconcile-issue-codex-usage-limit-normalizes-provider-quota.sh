...
test -f "$runs_root/fl-issue-88/issue-comment.md"
grep -q '^# Blocker: Provider quota is currently exhausted$' "$runs_root/fl-issue-88/issue-comment.md"
grep -q 'current Codex usage cap for the active account' "$runs_root/fl-issue-88/issue-comment.md"
test -f "$posted_comment_file"
grep -q '^# Blocker: Provider quota is currently exhausted$' "$posted_comment_file"

echo "issue reconcile codex usage-limit normalization test passed"
