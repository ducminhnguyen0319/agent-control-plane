...
heartbeat_start_pr_review_worker() { printf 'PR:%s\n' "${1:-}" >>"${TEST_EVENTS_LOG:?}"; }
...
PROVIDER_KEY=openclaw-openrouter-stepfun-step-3.5-flash-free
KIND=provider
ITEM_ID=openclaw-openrouter-stepfun-step-3.5-flash-free
ATTEMPTS=2
NEXT_ATTEMPT_EPOCH=9999999999
NEXT_ATTEMPT_AT=2286-11-20T17:46:39Z
READY=no
...
test ! -f "$events_log" || test ! -s "$events_log"
grep -q '^PROVIDER_COOLDOWN$' <<<"$output"
grep -q '^BACKEND=openclaw$' <<<"$output"
grep -q '^MODEL=openrouter/stepfun/step-3.5-flash:free$' <<<"$output"
grep -q '^LAST_REASON=provider-quota-limit$' <<<"$output"
if grep -q '^LAUNCHED_' <<<"$output"; then
  echo "heartbeat launched work while provider cooldown was active" >&2
  exit 1
fi

echo "heartbeat loop provider cooldown suppression test passed"
