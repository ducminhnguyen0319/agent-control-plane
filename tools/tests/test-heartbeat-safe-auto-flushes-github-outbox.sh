...
cat >"$flow_root/tools/bin/github-write-outbox.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
printf 'OUTBOX_FLUSHED=7\n'
EOF
...
chmod +x \
  ...
  "$flow_root/tools/bin/github-write-outbox.sh" \
...
F_LOSNING_CATCHUP_INTERVAL_SECONDS=999999 \
bash "$flow_root/tools/bin/heartbeat-safe-auto.sh" >"$log_file"

grep -q 'github-outbox flush start' "$log_file"
grep -q '^OUTBOX_FLUSHED=7$' "$log_file"
grep -q 'github-outbox flush end status=0' "$log_file"

echo "heartbeat-safe-auto flushes github outbox test passed"
