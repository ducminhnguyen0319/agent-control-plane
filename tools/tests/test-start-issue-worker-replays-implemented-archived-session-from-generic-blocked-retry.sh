...
if [[ "${1:-}" == "issue" && "${2:-}" == "view" ]]; then
  issue_id="${3:-0}"
  cat <<JSON
{"number":${issue_id},"title":"Issue ${issue_id}","body":"Retry host publish from generic blocked state.","url":"https://example.test/issues/${issue_id}","labels":[{"name":"agent-blocked"}],"comments":[]}
JSON
  exit 0
fi
exit 64
...
archive_dir="$history_root/${session}-20260402-120000"
mkdir -p "$archive_dir"
cat >"$archive_dir/run.env" <<'EOF'
ISSUE_ID=777
BRANCH=agent/alpha/issue-777-test
...
echo "start issue worker replays implemented archived session from generic blocked retry test passed"
