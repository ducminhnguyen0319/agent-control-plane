...
tmpdir="$(mktemp -d)"
fallback_dir="$(mktemp -d /tmp/acp-codex-fallback.XXXXXX)"
...
session="fl-codex-log-recovery-test"
...
recovered from logged fallback
COMMENT
...
grep -q '^recovered from logged fallback$' "$run_dir/issue-comment.md"

echo "agent-project codex session wrapper recovers logged artifacts test passed"
