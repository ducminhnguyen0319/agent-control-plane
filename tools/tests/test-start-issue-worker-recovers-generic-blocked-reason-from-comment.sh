...
if [[ "${1:-}" == "issue" && "${2:-}" == "view" ]]; then
  issue_id="${3:-0}"
  cat <<JSON
{"number":${issue_id},"title":"Blocked issue ${issue_id}","body":"Investigate blocked issue.","url":"https://example.test/issues/${issue_id}","labels":[{"name":"agent-blocked"}],"comments":[{"body":"# Blocker: Verification requirements were not satisfied\n\nHost publication stopped this cycle because the branch did not carry the required verification signal for a safe recurring issue PR.\n\nWhy it was blocked:\n- the verification guard could not confirm the expected checks for this change\n- recurring issue publication should stop rather than open an unverifiable PR","createdAt":"2026-03-15T10:00:00Z"}]}
JSON
  exit 0
fi
...
PATH="$shim_dir:$PATH" \
bash "$bin_dir/start-issue-worker.sh" 777 >/dev/null

prompt_file="$(find "$agent_root/runs" -maxdepth 2 -name prompt.md | head -n 1)"
test -n "$prompt_file"

grep -q 'Last recorded blocker: `verification-guard-blocked`.' "$prompt_file"
grep -q 'Add the missing verification or shrink the touched surface before attempting another publish cycle.' "$prompt_file"
grep -q '^# Blocker: Verification requirements were not satisfied$' "$prompt_file"

echo "start issue worker recovers generic blocked reason from comment test passed"
