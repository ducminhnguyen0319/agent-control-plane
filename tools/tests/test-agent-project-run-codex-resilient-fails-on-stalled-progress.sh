...
tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/codex-stalled-progress.XXXXXX")"
...
if [[ "${1:-}" == "exec" ]]; then
  printf '%s\n' '{"type":"thread.started","thread_id":"thread-stalled"}'
  sleep 3
  exit 0
fi
...
printf 'Stalled progress prompt\n' >"$prompt_file"
...
grep -q '^THREAD_ID=thread-stalled$' "$runner_env"
grep -q '^LAST_FAILURE_REASON=no-codex-progress-before-stall-threshold$' "$runner_env"

echo "agent-project-run-codex-resilient stalled progress test passed"
