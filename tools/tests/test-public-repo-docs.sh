...
grep -q '^| `codex` | Production-ready | Full ACP workflow support today\. |$' "$ROOT_DIR/README.md"
grep -q '^| `openclaw` | Production-ready | Full ACP workflow support, including resident-style runs\. |$' "$ROOT_DIR/README.md"
grep -q '^| `opencode` | Scaffolded | Routing and docs in place; live execution not yet implemented\. |$' "$ROOT_DIR/README.md"
grep -q '^| `kilo` | Scaffolded | Routing and docs in place; live execution not yet implemented\. |$' "$ROOT_DIR/README.md"
grep -q '^| `gemini-cli` | Roadmap | Strong future candidate; not wired into ACP yet\. |$' "$ROOT_DIR/README.md"
grep -q '^| `ollama` | Research | Candidate local-model substrate for future ACP integrations\. |$' "$ROOT_DIR/README.md"
...
grep -q 'The wizard auto-detects the current repo when possible' "$ROOT_DIR/README.md"
grep -q 'missing dependencies, prompts for `gh auth login`, scaffolds the profile' "$ROOT_DIR/README.md"
grep -q 'health checks, and optionally starts the runtime' "$ROOT_DIR/README.md"
...
grep -q '^| `setup \[--dry-run\] \[--json\]` | Guided bootstrap wizard\. Detects repo, installs deps, scaffolds profile, starts runtime\. `--dry-run` previews without writing anything\. `--json` emits a structured result\. |$' "$ROOT_DIR/README.md"
