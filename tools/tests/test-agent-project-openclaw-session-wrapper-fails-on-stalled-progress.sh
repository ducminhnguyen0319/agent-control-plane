...
session="fl-pr-openclaw-stalled-progress"
...
case "$cmd:$sub" in
  agents:add)
    ...
    ;;
  agent:*)
    printf '[agents/auth-profiles] synced openai-codex credentials from external cli\n'
    sleep 5
    ;;
  *)
...
grep -q '^\[openclaw\] stale-run no-agent-progress-before-stall-threshold elapsed=' "$run_dir/$session.log"
grep -q '^LAST_FAILURE_REASON=no-agent-progress-before-stall-threshold$' "$run_dir/runner.env"
grep -q '^RUNNER_STATE=failed$' "$run_dir/runner.env"

echo "agent-project openclaw session wrapper stalled progress test passed"
