...
if grep -q '^LAUNCH:101$' "$events_log"; then
  echo "waiting-provider resident controller issue unexpectedly relaunched" >&2
  exit 1
fi

echo "heartbeat loop waiting-provider resident controller does not block launches test passed"
