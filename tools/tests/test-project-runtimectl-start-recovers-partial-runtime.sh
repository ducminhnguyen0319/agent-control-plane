...
mkdir -p "${profile_dir}" "${fake_bin}" "${runs_root}" "${state_root}/resident-workers/issues/438"
...
cat >"${state_root}/resident-workers/issues/438/controller.env" <<EOF
ISSUE_ID=438
...
echo "project runtimectl start recovers partial runtime test passed"
