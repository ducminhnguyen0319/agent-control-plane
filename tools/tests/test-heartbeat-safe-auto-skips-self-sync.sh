...
platform_home="$tmpdir/platform-home"
runtime_home="$platform_home/runtime-home"
flow_root="$runtime_home/skills/openclaw/agent-control-plane"
bin_dir="$tmpdir/bin"
agent_root="$tmpdir/project"
memory_dir="$tmpdir/memory"
log_file="$tmpdir/heartbeat.log"
profile_home="$tmpdir/profiles"
ensure_log="$tmpdir/ensure-runtime-sync.log"

mkdir -p \
  "$flow_root/tools/bin" \
  "$flow_root/hooks" \
  "$runtime_home/skills/openclaw/codex-quota-manager/scripts" \
  "$bin_dir" \
  "$agent_root/runs" \
  "$agent_root/state" \
  "$memory_dir" \
...
#!/usr/bin/env bash
set -euo pipefail
printf 'scheduled-issue-catchup-ran\n'
EOF

cat >"$flow_root/tools/bin/ensure-runtime-sync.sh" <<EOF
#!/usr/bin/env bash
set -euo pipefail
printf 'unexpected-self-sync\n' >>"$ensure_log"
EOF
...
cat >"$runtime_home/skills/openclaw/codex-quota-manager/scripts/auto-switch.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x \
  ...
  "$flow_root/tools/bin/ensure-runtime-sync.sh" \
  "$flow_root/hooks/heartbeat-hooks.sh" \
  "$runtime_home/skills/openclaw/codex-quota-manager/scripts/auto-switch.sh" \
  "$bin_dir/tmux" \
  "$bin_dir/ps" \
  "$bin_dir/codex-quota"
...
SHARED_AGENT_HOME="$runtime_home" \
...
grep -q '^RUNTIME_SYNC_SKIPPED=active-runtime-home$' "$log_file"
if [[ -f "$ensure_log" ]]; then
  echo "heartbeat-safe-auto unexpectedly ran ensure-runtime-sync inside runtime-home" >&2
  exit 1
fi
grep -q 'shared heartbeat loop end status=0' "$log_file"
grep -q 'heartbeat-loop-ran' "$log_file"

echo "heartbeat-safe-auto skips self-sync test passed"
