...
grep -q '^CODEX_QUOTA_MANAGER_UNAVAILABLE=yes$' "$log_file"
grep -q '^CODEX_QUOTA_MANAGER_REASON=missing-codex-quota$' "$log_file"
grep -q 'codex quota preflight skipped unavailable (missing-codex-quota)' "$log_file"
grep -q 'shared heartbeat loop end status=0' "$log_file"
grep -q 'merged-pr catchup end status=0' "$log_file"

echo "heartbeat-safe-auto quota health signal test passed"
