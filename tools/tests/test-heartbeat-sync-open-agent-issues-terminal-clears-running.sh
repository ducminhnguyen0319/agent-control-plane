...
heartbeat_list_running_issue_ids() { printf '201\n202\n203\n204\n'; }
...
grep -q '^SYNC:201$' "$events_log"
grep -q '^SYNC:202$' "$events_log"
grep -q '^SYNC:204$' "$events_log"
if grep -q '^SYNC:203$' "$events_log"; then
  echo "running issue unexpectedly triggered label sync" >&2
  exit 1
fi

echo "heartbeat sync open agent issues terminal clears running test passed"
