...
#!/usr/bin/env bash
set -euo pipefail
printf 'quota-script-should-not-run\n'
exit 99
EOF
...
grep -q 'CODING_WORKER=openclaw' "$log_file"
grep -q 'CODEX_QUOTA_PREFLIGHT_SKIPPED_FOR_WORKER=yes' "$log_file"
grep -q 'DYNAMIC_CONCURRENCY_QUOTA_MODE=non-codex-bypass' "$log_file"
grep -q 'EFFECTIVE_MAX_CONCURRENT_WORKERS=20' "$log_file"
grep -q 'EFFECTIVE_MAX_CONCURRENT_PR_WORKERS=12' "$log_file"
grep -q -- '--max-concurrent-workers 20' "$log_file"
grep -q -- '--max-concurrent-pr-workers 12' "$log_file"
if grep -q 'quota-script-should-not-run' "$log_file"; then
  echo "quota script unexpectedly executed" >&2
  exit 1
fi
...
echo "heartbeat-safe-auto openclaw skips codex quota test passed"
