...
bash "$shared_bin/agent-project-reconcile-issue-session" \
  ...
  --hook-file "$tmpdir/issue-hook.sh" >"$output_file" 2>"$stderr_file"

grep -q '^STATUS=SUCCEEDED$' "$output_file"
grep -q 'issue cleanup warning session=demo-issue-321 status=23 mode=branch' "$stderr_file"
grep -q 'issue cleanup detail session=demo-issue-321 unable to prune worktree' "$stderr_file"

echo "agent-project-reconcile-issue-session warns on cleanup failure test passed"
