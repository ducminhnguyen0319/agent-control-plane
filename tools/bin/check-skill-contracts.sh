...
root_pr_reconcile="${SKILL_ROOT}/tools/bin/agent-project-reconcile-pr-session"
root_issue_reconcile="${SKILL_ROOT}/tools/bin/agent-project-reconcile-issue-session"

# Modules from flow-config-lib.sh split
flow_profile_lib="${SKILL_ROOT}/tools/bin/flow-profile-lib.sh"
flow_forge_lib="${SKILL_ROOT}/tools/bin/flow-forge-lib.sh"
flow_provider_lib="${SKILL_ROOT}/tools/bin/flow-provider-lib.sh"
flow_session_lib="${SKILL_ROOT}/tools/bin/flow-session-lib.sh"
flow_execution_lib="${SKILL_ROOT}/tools/bin/flow-execution-lib.sh"
...
check_contains "$run_opencode_session" "crush run" "opencode adapter crush run contract"
check_contains "$run_kilo_session" "kilo run" "kilo adapter kilo run contract"
check_contains "$flow_execution_lib" "flow_export_execution_env" "flow config lib exports execution env"
check_contains "$flow_provider_lib" "flow_resolve_provider_quota_cooldowns" "flow config lib provider quota cooldown resolver"
check_contains "$flow_session_lib" "flow_selected_provider_pool_env" "flow config lib provider pool selection helper"
check_contains "$flow_provider_lib" "flow_resolve_codex_quota_bin" "flow config lib codex quota bin resolver"
check_contains "$flow_provider_lib" "flow_resolve_codex_quota_manager_script" "flow config lib codex quota manager resolver"
...
# Check new modules from flow-config-lib.sh split
check_sync_if_present "$flow_profile_lib" "${RUNTIME_SKILL_ROOT}/tools/bin/flow-profile-lib.sh" "flow profile lib source/runtime sync"
check_sync_if_present "$flow_forge_lib" "${RUNTIME_SKILL_ROOT}/tools/bin/flow-forge-lib.sh" "flow forge lib source/runtime sync"
check_sync_if_present "$flow_provider_lib" "${RUNTIME_SKILL_ROOT}/tools/bin/flow-provider-lib.sh" "flow provider lib source/runtime sync"
check_sync_if_present "$flow_session_lib" "${RUNTIME_SKILL_ROOT}/tools/bin/flow-session-lib.sh" "flow session lib source/runtime sync"
check_sync_if_present "$flow_execution_lib" "${RUNTIME_SKILL_ROOT}/tools/bin/flow-execution-lib.sh" "flow execution lib source/runtime sync"
if (( HAS_DISTINCT_ROOT_SOURCE )); then
